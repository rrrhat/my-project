...
const API: {
  ALLROLE_URL: string
  ADDROLE_URL: string
  UPDATEROLE_URL: string
  ALLPERMISSTION: string
  SETPERMISSRION_URL: string
  REMOVEROLE_URL: string
} = {
  //获取全部的角色
  ...
  //给相应的职位分配权限
  SETPERMISSRION_URL: '/admin/acl/permission/doAssign',
  //删除已有的职位
...
export const reqSetPermisstion = (roleId: number, permissionId: number[]) =>
  request.post<any, any>(
    // 手动拼接，强制用逗号分隔，避免后端解析问题
    `${API.SETPERMISSRION_URL}?roleId=${roleId}&permissionId=${permissionId.join(',')}`
  )
//删除已有的职位
